...
function scanForTokens(
	repoRoot,
	{ tokens, exts, maxFiles = 2500, maxDepth = 8 },
) {
	...
	for (const filePath of files) {
		...
		for (let i = 0; i < loweredTokens.length; i += 1) {
			...
			if (haystack.includes(token))
				matches.set(token, path.relative(repoRoot, filePath));
		}
...
function detectPackageManager(repoRoot) {
	...
	const hasBun =
		existsFile(path.join(repoRoot, "bun.lockb")) ||
		existsFile(path.join(repoRoot, "bun.lock"));
...
function guessGutenbergVersion(repoRoot) {
	const gutenbergPackageJson = path.join(
		repoRoot,
		"packages",
		"plugins",
		"package.json",
	...
	for (const candidate of [gutenbergPackageJson, rootPackageJson]) {
		...
		try {
			...
			if (
				pkg?.name === "@wordpress/plugins" &&
				typeof pkg?.version === "string"
			) {
				return {
					value: pkg.version,
					source: path.relative(repoRoot, candidate),
				};
			}
...
function detectConfigConstants(repoRoot) {
	const { results: configFiles } = findFilesRecursive(
		repoRoot,
		(p) => path.basename(p) === "wp-config.php",
		{
			maxFiles: 4000,
			maxDepth: 4,
		},
	...
	if (!contents)
		return { source: path.relative(repoRoot, configPath), constants: {} };

	const c = contents;
	const enabled = (name) =>
		new RegExp(
			`define\\(\\s*['"]${name}['"]\\s*,\\s*(true|1)\\s*\\)`,
			"i",
		).test(c) || new RegExp(`\\b${name}\\b\\s*=\\s*(true|1)`, "i").test(c);
...
function detectKinds(repoRoot, signals) {
	...
	if (signals.detectedThemeName)
		kinds.add(signals.isBlockTheme ? "wp-block-theme" : "wp-theme");
	if (signals.detectedPluginName)
		kinds.add(signals.isBlockPlugin ? "wp-block-plugin" : "wp-plugin");
...
function buildRecommendations({
	...
	if (tooling.node.hasPackageJson) {
		...
		const hasScript = (name) =>
			Boolean(
				packageJson?.scripts &&
					Object.prototype.hasOwnProperty.call(packageJson.scripts, name),
			);
		...
		if (tooling.node.usesWordpressScripts)
			notes.push(
				"Detected @wordpress/scripts usage; prefer its standard lint/build/test scripts.",
			);
	}
	...
	if (tooling.tests.hasWpEnv)
		notes.push("Detected wp-env; E2E workflows may rely on Docker.");
	if (signals.scanTruncated)
		notes.push(
			"Scan truncated due to file limit; some signals may be missing.",
		);
	if (primaryKind === "unknown")
		notes.push(
			"Could not confidently classify repo; inspect root for plugin/theme headers or wp-content structure.",
		);
...
function main() {
	const repoRoot = "F:\\host\\www\\gutenberg";
	...
	const isWpCoreCheckout = existsFile(
		path.join(repoRoot, "wp-includes", "version.php"),
	);
	const isGutenbergRepo =
		existsDir(path.join(repoRoot, "packages")) &&
		(existsDir(path.join(repoRoot, "packages", "block-editor")) ||
			existsDir(path.join(repoRoot, "packages", "components")));
	...
	const pkgHasInteractivity = Boolean(
		packageJson?.devDependencies?.["@wordpress/interactivity"] ||
			packageJson?.dependencies?.["@wordpress/interactivity"],
	);
	const pkgHasAbilities = Boolean(
		packageJson?.devDependencies?.["@wordpress/abilities"] ||
			packageJson?.dependencies?.["@wordpress/abilities"],
	);
	...
	for (const entry of fs.readdirSync(repoRoot, { withFileTypes: true })) {
		...
		if (entry.name.toLowerCase().endsWith(".php"))
			pluginCandidates.push(path.join(repoRoot, entry.name));
		if (entry.name === "style.css")
			themeCandidates.push(path.join(repoRoot, entry.name));
	}
	...
	const { results: blockJsonFiles, truncated: scanTruncated } =
		findFilesRecursive(repoRoot, (p) => path.basename(p) === "block.json", {
			maxFiles: 6000,
			maxDepth: 8,
		});
	const { results: themeJsonFiles } = findFilesRecursive(
		repoRoot,
	...
	const isBlockTheme =
		themeJsonFiles.length > 0 &&
		templatesDirCandidates.some((p) => existsDir(p));
	...
	const { results: wpCliConfigFiles, truncated: wpCliConfigTruncated } =
		findFilesRecursive(
			repoRoot,
	...
	const composerRequire =
		composerJson?.require && typeof composerJson.require === "object"
			? composerJson.require
			: {};
	const composerRequireDev =
		composerJson?.["require-dev"] &&
		typeof composerJson["require-dev"] === "object"
			? composerJson["require-dev"]
			: {};
	...
	const usesInteractivityApi =
		pkgHasInteractivity || Object.keys(interactivityScan.matches).length > 0;
	const usesAbilitiesApi =
		pkgHasAbilities || Object.keys(abilitiesScan.matches).length > 0;
	const usesInnerBlocks = Object.keys(innerBlocksScan.matches).length > 0;
	const usesWpCli =
		composerHasWpCli ||
		wpCliConfigFiles.length > 0 ||
		Object.keys(wpCliTokenScan.matches).length > 0;

	const wpContentRoot = path.join(repoRoot, "wp-content");
	const hasObjectCacheDropin = existsFile(
		path.join(wpContentRoot, "object-cache.php"),
	);
	const hasAdvancedCacheDropin = existsFile(
		path.join(wpContentRoot, "advanced-cache.php"),
	);
	...
	const hasQueryMonitorPlugin = existsDir(
		path.join(wpContentRoot, "plugins", "query-monitor"),
	);
	const hasPerformanceLabPlugin = existsDir(
		path.join(wpContentRoot, "plugins", "performance-lab"),
	);
	...
	const hasWpEnv =
		...
		Boolean(
			packageJson?.devDependencies?.["@wordpress/env"] ||
				packageJson?.dependencies?.["@wordpress/env"],
		);
	...
	const hasPhpUnit =
		phpunitXml.length > 0 ||
		Boolean(
			composerJson?.requireDev?.phpunit ||
				composerJson?.["require-dev"]?.phpunit,
		);
