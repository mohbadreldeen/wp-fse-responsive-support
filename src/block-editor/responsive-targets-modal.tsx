...
import { Modal, Button, SearchControl, CheckboxControl, Notice } from '@wordpress/components';
...
const HeaderToolbarButton = ( editPost as { PluginToolbarButton?: any } )?.PluginToolbarButton;

const runtimeSettings = ( ( window as Window & {
	responsiveOverridesSettings?: RuntimeSettings;
} )?.responsiveOverridesSettings || {} ) as RuntimeSettings;
...
export const ResponsiveTargetsModal = () => {
	...
	useEffect( () => {
		...
		const mountFallbackButton = () => {
			...
			buttonEl.setAttribute( 'aria-label', __( 'Responsive Overrides', 'responsive-overrides' ) );
	...
	const discovered = useMemo<DiscoverableBlock[]>( () => {
		return blockTypes
			.map( ( block: BlockType ) => {
				const attrs = listAttributeCandidates( block.attributes || {} ) as ResponsiveTarget[];
				return {
					name: block.name,
	...
	useEffect( () => {
		...
		( normalizeTargets( getActiveTargets() ) as ResponsiveTarget[] ).forEach( ( target: ResponsiveTarget ) => {
	...
	const filteredBlocks = useMemo<DiscoverableBlock[]>( () => {
		...
		return discovered
			.map( ( block: DiscoverableBlock ) => {
				...
				const attrMatches = block.attributes.filter( ( attr: ResponsiveTarget ) => attr.path.toLowerCase().includes( term ) );
	...
	const saveTargets = async () => {
		...
		try {
			...
			const nextTargets = setActiveTargets( response?.targets || [] ) as ResponsiveTarget[];
			...
			{ isOpen && (
				...
				>
					<SearchControl
						...
						onChange={ setSearch }
					/>
					<p>{ __( 'Select block attributes to make responsive per device.', 'responsive-overrides' ) }</p>
					<p>{ `${ selectedCount } ${ __( 'attributes selected', 'responsive-overrides' ) }` }</p>
					<div style={ { maxHeight: '50vh', overflow: 'auto', border: '1px solid #ddd', padding: '8px' } }>
						{ filteredBlocks.map( ( block: DiscoverableBlock ) => (
							<div key={ block.name } style={ { marginBottom: '16px' } }>
								<strong>{ block.title }</strong>
								<code style={ { display: 'block', marginBottom: '6px' } }>{ block.name }</code>
					...
					{ feedback && <Notice status={ feedback.status } isDismissible={ false }>{ feedback.message }</Notice> }

					<div style={ { marginTop: '16px', display: 'flex', gap: '8px' } }>
						<Button variant="primary" onClick={ saveTargets } isBusy={ isSaving } disabled={ isSaving }>
